...
const HKDF_INFO = 'Cipher-ECC-Session';

/* BIP39-inspired word list (2048 common English words subset) */
const MNEMONIC_WORDS = [
  ...
  'among','amount','amused','anchor','ancient','anger','angle','annual','another','answer',
  'antenna','apple','approve','arch','arctic','area','arena','army','arrive','arrow',
  'art','artist','aspect','assault','asset','attract','auction','audit','august','aunt',
  'author','auto','avocado','avoid','awake','aware','awesome','awful','axis','baby',
  'bachelor','bacon','badge','bag','balance','balcony','ball','bamboo','banana','banner',
  'bar','barrel','base','basic','basket','battle','beach','bean','beauty','become',
  'before','begin','behave','behind','believe','below','bench','benefit','best','betray',
  'better','between','beyond','bicycle','bird','birth','bitter','black','blade','blame',
  'blank','blast','bleak','bless','blind','blood','blossom','blow','blue','blur',
  'blush','board','boat','body','bomb','bone','bonus','book','boost','border',
  'boring','borrow','boss','bottom','bounce','box','brain','brand','brave','bread',
  'breeze','brick','bridge','brief','bright','bring','broken','brother','brown','brush',
  'bubble','buddy','budget','buffalo','build','bullet','bundle','burden','burger','burst',
  'bus','butter','buyer','cabin','cable','cactus','cage','cake','call','calm',
  'camera','camp','candy','cannon','canvas','canyon','capable','capital','captain','carbon',
  'card','cargo','carpet','carry','cart','case','cash','castle','casual','catalog',
  'catch','category','cattle','caught','cause','caution','cave','ceiling','celery','cement',
  'census','century','cereal','chair','chalk','champion','change','chaos','chapter','charge',
  'chase','cheap','check','cheese','chest','chicken','chief','child','chimney','choice',
  'chunk','circle','citizen','city','civil','claim','clap','clarify','claw','clay',
  'clean','clerk','clever','cliff','climb','clinic','clip','clock','close','cloud',
  'clown','club','cluster','clutch','coach','coast','coconut','code','coffee','coil',
  ...
  'cushion','custom','cycle','dad','damage','damp','dance','danger','daring','dash',
  ...
  'easy','echo','ecology','economy','edge','edit','edition','editor','educate','effort',
  ...
  'extend','extra','eye','fabric','face','faculty','fade','faint','faith','fall',
  'false','fame','family','famous','fan','fancy','fantasy','farm','fashion','fat',
  'fatal','father','fatigue','fault','favorite','feature','federal','fee','feed','feel',
  ...
  'flee','flight','flip','float','flock','floor','flower','fluid','flush','foam',
  ...
  'gun','gym','habit','half','hammer','hamster','hand','happy','harbor','hard',
  ...
  'hybrid','ice','icon','idea','identify','idle','ignore','image','imitate','immense',
  'immune','impact','impose','improve','impulse','inch','include','income','increase','index',
  'indicate','indoor','industry','infant','inflict','inform','initial','inject','inmate','inner',
  'innocent','input','inquiry','insane','insect','inside','inspire','install','intact','interest',
  'into','invest','invite','iron','island','isolate','issue','item','ivory','jacket',
  ...
  'push','puzzle','pyramid','quality','quantum','quarter','question','quick','quit','quiz',
  ...
  'soda','soft','solar','soldier','solid','solution','someone','song','soon','sorry',
...
export function generateMnemonic(): string[] {
  ...
  for (let i = 0; i < 12; i++) {
    words.push(MNEMONIC_WORDS[arr[i] % MNEMONIC_WORDS.length]);
  }
  return words;
}

/** Derive a deterministic ECDH key pair from 12 mnemonic words */
export async function deriveKeyPairFromMnemonic(words: string[]): Promise<{ keyPair: CryptoKeyPair; publicKeyB64: string }> {
  const phrase = words.join(' ');
  const encoded = new TextEncoder().encode(phrase);
  const hash = await crypto.subtle.digest('SHA-256', encoded);
  const seed = new Uint8Array(hash);

  // Import seed as HKDF key, then derive raw P-256 private key material
  const hkdfKey = await crypto.subtle.importKey('raw', seed, 'HKDF', false, ['deriveBits']);
  const derivedBits = await crypto.subtle.deriveBits(
    { name: 'HKDF', hash: 'SHA-256', salt: new TextEncoder().encode('Cipher-Mnemonic'), info: new TextEncoder().encode('ECDH-P256') },
    hkdfKey,
    256
  );

  // Import as PKCS8 is not possible directly for ECDH; use JWK with raw d value
  // Map the 256-bit seed to a valid P-256 private scalar in [1, n-1].
  const order = p256.Point.Fn.ORDER;
  const scalar = (bytesToNumberBE(new Uint8Array(derivedBits)) % (order - 1n)) + 1n;
  const dBytes = numberToBytesBE(scalar, 32);

  // WebCrypto can't derive an EC public point from the private scalar alone
  // (importing an EC private JWK requires x,y, which subtle won't compute from
  // d). Compute the point with the audited @noble/curves implementation, then
  // import a complete JWK. This only enables deterministic paired-mode identity
  // recovery — AES-256-GCM / PBKDF2 and the message envelope are untouched.
  const pubPoint = p256.getPublicKey(dBytes, false); // 0x04 || x(32) || y(32)
  ...
  return { keyPair: { privateKey, publicKey }, publicKeyB64: toBase64(new Uint8Array(pubPoint)) };
}
...
export async function generateEccKeyPair(): Promise<CryptoKeyPair> {
...
async function importPublicKeyB64(publicKeyB64: string): Promise<CryptoKey> {
...
export function fingerprintPublicKey(publicKeyB64: string): string {
  const trimmed = publicKeyB64.replace(/\s+/g, '');
  if (!trimmed) return '';
  return `${trimmed.slice(0, 10)}...${trimmed.slice(-10)}`;
}

export async function deriveSharedSessionSecret(
  myPrivateKey: CryptoKey,
  friendPublicKeyB64: string,
  myPublicKeyB64: string
): Promise<{ secret: string; saltB64: string }> {
  const friendPublicKey = await importPublicKeyB64(friendPublicKeyB64);
  ...
  // Deterministic salt: SHA-256(sort(pubA, pubB)) — both sides produce the same salt
  const sorted = [myPublicKeyB64, friendPublicKeyB64].sort();
  const saltInput = new TextEncoder().encode(sorted[0] + '|' + sorted[1]);
  ...
  return {
    secret: toBase64(new Uint8Array(finalBits)),
